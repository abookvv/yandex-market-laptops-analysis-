...
def yand(req, driver, df):
    ...
    # будет вылезать ошибка если у товара нет рейтинга и оценок, но нам такие не нужны, поэтому 
    for item in items:
        try:
            # Название товара
            ...
            # Иноформация о ноутбуке
            div_info = item.find_all('div', class_='_2Ce4O')
            dict_info = dict()
            for i in div_info:
                info = i.find_all('span')
                dict_info[info[0].text.strip()] = info[1].text.strip()
        ...
        df.loc[len(df)] = [title, dict_info, price, rating, reviews, "yand", full_link, req]
